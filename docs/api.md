...
> **`DEBUG=true` 일 때만 서버에 등록됩니다.**
> 이 엔드포인트들은 인증이 없고, 그중 `/api/demo/reset` 은 모든 관측·경보·기기와
> **페어링 세션 전체**를 삭제합니다. 공개된 배포에 노출되면 누구든 URL만 알면
> 벌통 기록을 지우고 모든 기기의 연결을 끊을 수 있으므로, production
> (`DEBUG=false`) 에서는 라우터 자체를 mount하지 않습니다. 이때 모든
> `/api/demo/*` 요청은 **404** 를 반환합니다.
