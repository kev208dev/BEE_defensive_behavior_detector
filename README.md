...
기기 밖으로 나가지 않아 사생활 보호가 좋아집니다. 영상 경로에서 서버로 가는 것은 수백 byte
수준의 탐지 metadata뿐입니다. (음향 분석은 여전히 서버에서 수행하므로 3초 audio chunk는
그대로 업로드됩니다 — 기기를 벗어나지 않는 것은 **영상**입니다.)
이 덕분에 기존 수십 KB JPEG/초 방식보다 대역폭과 Railway 처리 비용이 크게 줄고, 네트워크
